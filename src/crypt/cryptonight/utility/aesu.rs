pub fn aes_round_key(prev: &[u8; 16], prev_prev: &[u8; 16], round: u8) -> [u8; 16] {
    let mut result = [0; 16];
    for i in 0..16 {
        result[i] = prev[i] ^ prev_prev[i] ^ round;
    }
    result
}

// Expand 10 round keys from 32-byte AES key
pub fn e10rk(key: &[u8; 32]) -> [[u8; 16]; 10] {
    let mut round_keys = [[0; 16]; 10];
    round_keys[0].copy_from_slice(&key[0..16]);
    round_keys[1].copy_from_slice(&key[16..32]);
    for i in 2..10 {
        round_keys[i] = aes_round_key(&round_keys[i - 2], &round_keys[i - 1], i as u8);
    }
    round_keys
}

pub fn sub_bytes(block: &[u8; 16]) -> [u8; 16] {
    let mut result = [0; 16];
    for i in 0..16 {
        result[i] = SBOX[block[i] as usize];
    }
    result
}

pub fn shift_rows(block: &[u8; 16]) -> [u8; 16] {
    let mut result = [0; 16];
    result[0] = block[0];
    result[1] = block[5];
    result[2] = block[10];
    result[3] = block[15];
    result[4] = block[4];
    result[5] = block[9];
    result[6] = block[14];
    result[7] = block[3];
    result[8] = block[8];
    result[9] = block[13];
    result[10] = block[2];
    result[11] = block[7];
    result[12] = block[12];
    result[13] = block[1];
    result[14] = block[6];
    result[15] = block[11];
    result
}

pub fn mix_columns(block: &[u8; 16]) -> [u8; 16] {
    let mut result = [0; 16];
    for i in 0..4 {
        let a = block[i * 4];
        let b = block[i * 4 + 1];
        let c = block[i * 4 + 2];
        let d = block[i * 4 + 3];
        result[i * 4] = FF_MUL2[a as usize] ^ FF_MUL3[b as usize] ^ c ^ d;
        result[i * 4 + 1] = a ^ FF_MUL2[b as usize] ^ FF_MUL3[c as usize] ^ d;
        result[i * 4 + 2] = a ^ b ^ FF_MUL2[c as usize] ^ FF_MUL3[d as usize];
        result[i * 4 + 3] = FF_MUL3[a as usize] ^ b ^ c ^ FF_MUL2[d as usize];
    }
    result
}

pub fn xor_with_key(block: &[u8; 16], round_key: &[u8; 16]) -> [u8; 16] {
    let mut result = [0; 16];
    for i in 0..16 {
        result[i] = block[i] ^ round_key[i];
    }
    result
}


pub fn aes_round(block: &[u8; 16], round_key: &[u8; 16]) -> [u8; 16] {
    let result = sub_bytes(block);
    let result = shift_rows(&result);
    let result = mix_columns(&result);
    let result = xor_with_key(&result, round_key);
    result
}

// Here goes the magical numbers

const SBOX: [u8; 256] = [
    
];

pub const FF_MUL2: [u8; 256] = [
    
];

pub const FF_MUL3: [u8; 256] = [
    
];